...
2026-09-08、ユーザー端末で実準備が成功。workspaceは/dev/loop0、executorは/dev/loop1のext4で、指定容量以下・nosuid/nodevを確認しました。両mountと専用user managerがactiveで、user@1001.serviceのRequires/Afterに両mountが設定されています。OS再起動後とENOSPCの受入は未完了です。次は[実コンテナの一括検証](PROGRAM.md)で容量境界とコンテナ動作を確認します。
