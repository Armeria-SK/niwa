#!/bin/sh
set -eu
test "$#" -ge 1 && test "$#" -le 2 && test "$1" = --apply || { echo 'Usage: sudo sh verify-extensions.sh --apply [sha256:EXISTING_BROWSER_IMAGE]' >&2; exit 2; }
test "$(id -u)" -eq 0 || { echo 'Run with sudo.' >&2; exit 1; }
root=/home/niwa/niwa
executor_uid=$(id -u niwa-exec)
python3 "$root/deploy/ubuntu/prepare-browser-seccomp.py" --apply
systemctl is-active --quiet niwa.service niwa-workspace.service "user@$executor_uid.service"
# Refresh supplementary IPC membership without changing directory permissions.
runuser -u niwa -- /usr/bin/node "$root/deploy/ubuntu/verify-services.mjs"
runuser -u niwa-exec -- env -i PATH=/usr/bin:/bin HOME="$root/runtime/executor/home" \
  XDG_RUNTIME_DIR="/run/user/$executor_uid" DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/$executor_uid/bus" \
  systemd-run --user --wait --pipe --collect --property=Delegate=yes --property=MemoryMax=3G --property=TasksMax=1024 \
  --setenv="HOME=$root/runtime/executor/home" --setenv="XDG_RUNTIME_DIR=/run/user/$executor_uid" \
  /usr/bin/node "$root/deploy/ubuntu/prepare-browser.mjs" "$@"
sh "$root/deploy/ubuntu/verify-packages.sh" --apply
