...
try {
  ...
  const receipt = {image,reference:suppliedImage ? image : lock.reference,verified_at:new Date().toISOString(),
    checks:['browser-chroot-seccomp','container-boundaries','sandbox-render','local-dom-interaction-no-network','broker-only-resources','form-prepare-no-send','exact-approval-synthetic-send-reopen','stale-reference','cleanup']};
