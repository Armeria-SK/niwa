...
export class AutonomousWakes {
  ...
  list(actor: Actor) {
    ...
    return this.db.prepare(`SELECT a.id AS agent_id,a.name,a.status,w.next_at,w.reason,w.task_id,t.state,t.provider_retry_at,
      EXISTS(SELECT 1 FROM tasks busy WHERE busy.agent_id=a.id AND busy.state NOT IN ('completed','failed','cancelled')) AS busy,
      EXISTS(SELECT 1 FROM approval_requests p JOIN tasks t ON t.id=p.task_id WHERE t.agent_id=a.id AND p.status='pending') AS approval
      ...
      WHERE a.id NOT IN (SELECT id FROM deleted_agents) ORDER BY a.rowid`).all().map(row => ({
        ...
        reason: settings.paused ? '全体停止中' : !settings.autonomous ? '自発活動オフ' : row.status !== 'active' ? '休眠中'
          : roomBlocked ? '利用できる共有会話がありません' : row.approval ? '承認待ち' : row.busy ? (row.task_id ? '前回の活動を継続・待機中' : '既存の仕事を優先') : row.reason ?? '起動判定の準備中',
        next_at: settings.paused || !settings.autonomous || row.status !== 'active' || roomBlocked || row.approval ? null
  ...
  dispatch(actor: Actor, now = Date.now()): void {
    ...
    transaction(this.db, () => {
      ...
      for (const row of this.db.prepare(`SELECT w.*,t.state,t.result,t.updated_at FROM autonomous_wakes w JOIN tasks t ON t.id=w.task_id
        WHERE t.state IN ('completed','failed','cancelled')`).all()) {
        const failed = row.state !== 'completed', failures = failed ? Number(row.failures)+1 : 0;
        const rested = row.result === '今回は休息しました。';
        const delay = failed ? Math.min(360,60 * 2 ** Math.min(failures-1,3)) : rested ? 60 : 15;
        this.db.prepare('UPDATE autonomous_wakes SET task_id=NULL,next_at=?,failures=?,reason=? WHERE agent_id=?')
          .run(Number(row.updated_at)+delay*MINUTE,failures,failed?'失敗後の待機':rested?'休息中':'活動完了後の間隔',row.agent_id!);
      }
      const settings = this.db.prepare('SELECT paused,autonomous FROM settings WHERE id=1').get()!;
      ...
      const candidate = this.db.prepare(`SELECT w.agent_id FROM autonomous_wakes w JOIN agents a ON a.id=w.agent_id
        WHERE w.task_id IS NULL AND w.next_at<=? AND a.status='active' AND a.id NOT IN (SELECT id FROM deleted_agents)
        AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.agent_id=a.id AND t.state NOT IN ('completed','failed','cancelled'))
        AND NOT EXISTS(SELECT 1 FROM approval_requests p JOIN tasks t ON t.id=p.task_id WHERE t.agent_id=a.id AND p.status='pending')
      ...
      this.db.prepare("UPDATE autonomous_wakes SET task_id=?,last_started_at=?,reason='予定なしの定期判定から起動' WHERE agent_id=?")
        .run(task.id,now,candidate.agent_id!);
    });
  }
}
