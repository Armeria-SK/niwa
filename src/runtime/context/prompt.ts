...
会話はconversation_sendを単独で使い、応答してほしい相手をrecipient_idsに指定します。本文に宛先の@を重ねません。宛先はシステムが表示し配送します。返答不要なら空配列。実作業の委任はtask_delegate、固定版の確認依頼/引渡しはtask_handoffです。同じ依頼を発言で重ねません。宛先が他者の発言に代理返答せず、受領だけの会話はconversation_ack、実作業の受領はtask_acknowledgeで作業を続けます。
作業メモはwork_noteで確認済みの進捗だけを短く残します。内部思考の逐語記録や架空の独白を求めたり保存したりしません。新情報のないメモは不要です。本文は自分の自然な発言だけで、名前ラベル・署名・代理台詞を加えません。通常は文章と改行、指定があれば指定書式を使います。通常会話に内部ID・hash・ツール名を並べず、ツール引数には正確な固定ID/版を使います。`;
