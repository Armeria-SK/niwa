...
export class Workareas {
 ...
 authorize(actor:Actor,id:string,lease?:TaskLease):Area{
  ...
  if(who.kind!=='admin'){
   check(lease&&lease.task.agent_id===who.id&&lease.task.room_id===area.room_id&&this.runtime.tasks.active(actor,lease),'forbidden','Current task and conversation required');
  }
  return area;
 }
