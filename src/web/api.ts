...
export function createApiServer(runtime: Runtime, auth: WebAuth, models = new ModelGateway(runtime), webRoot?: string, backups?: Backups,
  ...
  const routes: Route[] = [
    ...
    {method:'POST',path:/^\/api\/workareas\/([0-9a-f-]{36})\/environments\/([a-f0-9]{64})\/activate$/,schema:object({expected_environment:Type.Union([Type.Null(),Type.String({pattern:'^[a-f0-9]{64}$'})])}),run:(m,b)=>scoped(m[1]!,{operation:'environment_activate',environment:m[2]!,expected_environment:b.expected_environment as string|null})},
    ...
    { method: 'GET', path: /^\/api\/rooms\/([0-9a-f-]{36})\/work-notes$/, run: m => ({notes:runtime.workNotes(admin,m[1]!)}) },
    { method: 'GET', path: /^\/api\/rooms\/([0-9a-f-]{36})\/coordination$/, run: m => ({tasks:runtime.coordination(admin,m[1]!),digest:runtime.coordinationDigest(admin,m[1]!)}) },
