...
export function createApiServer(runtime: Runtime, auth: WebAuth, models = new ModelGateway(runtime), webRoot?: string, backups?: Backups) {
  ...
  const routes: Route[] = [
    ...
    { method: 'GET', path: /^\/api\/state$/, run: () => ({ agents: runtime.agents(admin).map(agent => ({ ...agent, profile: runtime.profile(admin, agent.id) })),
      rooms: runtime.rooms(admin).map(room => ({ ...room, ...runtime.roomPreferences(admin, room.id), participants: runtime.participants(admin, room.id) })), settings: runtime.settings(admin), tasks: runtime.tasks.list(admin).map(task => ({ ...task, replies: runtime.tasks.replies(admin, task.id) })) }) },
    ...
    { method: 'PATCH', path: /^\/api\/settings$/, schema: object({ paused: Type.Optional(Type.Boolean()), generatedLimit: Type.Optional(Type.Integer({ minimum: 0 })),
