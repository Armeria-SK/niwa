...
export interface WorkareaRequest {area:string;epoch:string;operation:string;path?:string;revision?:string;expected_revision?:string|null;content?:string;encoding?:'utf8'|'base64';operation_id?:string;allow_start?:boolean;command?:string[];seconds?:number;candidate?:string;artifact?:string}
type Run=(root:string,request:ProgramRequest,signal:AbortSignal|undefined,name:string)=>Promise<ProgramOutput>;
...
export class WorkareaStore {
 ...
 constructor(private root:string,private run:Run,private cleanup:(name:string)=>Promise<void>){
 ...
 async execute(input:WorkareaRequest,signal?:AbortSignal):Promise<JsonObject>{
  ...
  if(input.operation==='purge'){
   ...
   if(this.busy.has(input.area))return {error:'busy'};
  ...
  if(!['write','run','publish'].includes(input.operation)||!input.operation_id||!uuid.test(input.operation_id))throw new WorkspaceError('unsupported');
  ...
  if(input.operation==='run'&&this.busy.has(input.area))return {error:'busy'};
  const container=`niwa-program-${randomUUID()}`,stage=join(this.root,'runs',id);
  this.db.prepare('INSERT INTO receipts VALUES (?,?,?,?,NULL)').run(id,input.area,encoded,input.operation==='run'?container:null);
  ...
  try{
   if(input.operation==='publish'){
    ...
    if(input.operation==='write'){
     ...
     const output=await this.run(stage,{command:input.command??[],seconds:input.seconds??0},signal,container);
     signal?.throwIfAborted();const candidate=this.save(area.base,stage);
     result={...output,candidate,base_revision:area.revision}; // Authorization and CAS happen separately after execution.
    }
   }
  }catch(error){result={error:error instanceof WorkspaceError?error.code:'outcome_unknown'};}
  finally{if(input.operation==='run')this.busy.delete(input.area);rmSync(stage,{recursive:true,force:true});}
