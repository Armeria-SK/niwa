...
function Presence({ members, paused, onPause, onAppearance, onMember, isPrivate, noHeading = false }) {
  ...
  return <div className="presence-content">
    ...
    <div className="presence-members">{members.map(member => <div key={member.id} className={`presence-member ${member.status === 'sleeping' ? 'sleeping' : ''}`}>
      ...
      {openJobs[member.id]&&member.jobs?.length ? <div className="presence-job-list" id={`jobs-${member.id}`}>{member.jobs.map(job=><div key={job.id}><span>{job.progress?.work_acknowledged?'受領済み・':''}{workLabel(job,paused)}</span><WorkFacts task={job} compact/><details className="presence-job-request"><summary><span className="request-preview">{shortWork(job.title)}</span><span className="request-open-label">依頼全文</span></summary><p>{job.prompt||job.title}</p></details></div>)}</div>:null}{member.status === 'sleeping' ? <MoonIcon size={19} className="sleep-icon" /> : null}
    </div>)}</div>
  </div>;
}
