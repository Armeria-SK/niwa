...
export function ThreadWorkList({groups,members,paused,onOpen,onThread}) {
 const purpose=p=><section className="work-purpose" key={p.id}><h3>{shortWork(p.title)}</h3><p className="muted">担当：{p.members.map(id=>members[id]?.name||'削除したBot').join('・')}</p>
 ...
 const assignment=task=><div className="work-assignment" key={task.id}><div><span className="muted">{members[task.member]?.name||'削除したBot'} · {label(task,paused)}</span><p>{shortWork(task.title)}</p><WorkFacts task={task} compact/><p className="muted">{recent(task)}</p>{task.reason&&<p className="task-reason">{task.reason}</p>}</div><button className="button secondary" onClick={()=>onOpen(task.id)}>詳細・操作</button></div>;
 return <div className="activity-list work-threads">{groups.map(group=><details className="work-thread" key={group.id}>
  <summary><div><h2>{group.title}</h2><p>目的：{group.purposes.slice(0,2).map(p=>shortWork(p.title)).join(' ／ ')}{group.purposes.length>2?` ほか${group.purposes.length-2}件`:''}</p><p className="muted">担当：{group.members.map(id=>members[id]?.name||'削除したBot').join('・')}</p><p className="muted">{group.active.length?`進行・待機中 ${group.active.length}件の仕事`:'終了した仕事の履歴'} · {group.active.length?[...new Set(group.active.flatMap(p=>p.active).map(t=>label(t,paused)))].slice(0,3).join('・'):label(group.latest,paused)}</p><p className="muted">直近：{members[group.latest.member]?.name} · {recent(group.latest)}</p></div></summary>
