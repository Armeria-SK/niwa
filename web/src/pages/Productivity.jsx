...
import { Avatar, EmptyState, Modal, Segmented, Switch } from '../components.jsx';
...
export function ArtifactLibrary({ onDelete, artifacts, members, onOpen, onThread }) {
  ...
  const matches=item=>(kind==='all'||kindLabel(item.kind)===kind)&&(owner==='all'||item.member===owner)&&(!search||current?.ids?.has(item.id)||`${item.name} ${item.description} ${members[item.member]?.name}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return <section>
    <SettingDisclosure title="成果物の品質確認" value={qualityBusy?'保存中…':quality?quality.enabled?'有効':'無効':qualityError?'取得失敗':'読込中…'} busy={qualityBusy}>{quality?<Switch label="重要な成果物の品質確認" checked={quality.enabled} disabled={qualityBusy} onChange={changeQuality}/>:null}<p className="muted">新しい仕事から利用します。軽い会話にはレビュー待ちを追加しません。</p>{qualityError?<p role="alert">{qualityError}</p>:null}</SettingDisclosure>{workspaceOpen ? <WorkspaceFiles onClose={() => setWorkspaceOpen(false)} /> : null}<div className="section-toolbar"><div><h2>成果物</h2><p className="muted">みんなが作ったものを、ここから。</p></div><div className="inline-actions"><span className="muted">{visible.length}件</span><button className="button secondary" onClick={() => setWorkspaceOpen(true)}>共有フォルダー</button></div></div>
    ...
    <Segmented className="artifact-kinds" label="成果物の種類" value={kind} onChange={setKind} options={[{ value: 'all', label: 'すべて' }, ...[...new Set(artifacts.map(item => kindLabel(item.kind)))].map(value => ({ value, label: value }))]} />
