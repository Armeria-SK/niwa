...
export function WorkareaSettings({value,onChange,area,onSelect}) {
  ...
  return <div className="form-stack">
    ...
    <SettingDisclosure title="作業場所の設定" value={busy?'保存中…':value.enabled?'有効':'無効'} busy={busy}>
      <Switch label="個人・案件の作業場所" checked={!!value.enabled} disabled={busy} onChange={enabled=>change('/workareas','PATCH',{enabled})}/>
      ...
      {project?<form className="form-stack" onSubmit={e=>{e.preventDefault();change('/workareas/projects','POST',project);}}>
        <label className="field"><span>案件名</span><input disabled={busy} required maxLength={100} value={project.name} onChange={e=>setProject({...project,name:e.target.value})}/></label>
        <label className="field"><span>関連する会話</span><select aria-label="関連する会話" value={project.room_id} disabled={busy||!!project.id} onChange={e=>setProject({...project,room_id:e.target.value,members:[]})}>{value.rooms.map(r=><option key={r.id} value={r.id}>{r.title}</option>)}</select></label>
        <fieldset><legend>共同編集するBot</legend>{value.agents.filter(a=>room?.visibility==='shared'||room?.participants.includes(a.id)).map(a=><label key={a.id} style={{display:'block'}}><input type="checkbox" disabled={busy} checked={project.members.includes(a.id)} onChange={e=>setProject({...project,members:e.target.checked?[...project.members,a.id]:project.members.filter(id=>id!==a.id)})}/>{a.name}</label>)}</fieldset>
        <div className="inline-actions"><button className="button primary" disabled={busy||!project.members.length}>設定を保存</button><button type="button" className="button subtle" disabled={busy} onClick={()=>setProject(null)}>取消</button></div>
      </form>:null}
      <p className="field-hint">個人領域はBotが仕事中に選ぶと作成されます。管理者はすべて閲覧できます。案件から外したBotは以後の取得と変更ができません。</p>
    </SettingDisclosure>{error?<p role="alert" className="field-error">{error}</p>:null}
  </div>;
}
