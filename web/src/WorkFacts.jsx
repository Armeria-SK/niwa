...
export function WorkFacts({task,compact=false}) {
 ...
 return <div className="work-facts">
  ...
  {!compact&&<><p>最終活動・状態更新：{progress?.last_activity_at||task.updated_at?time(progress?.last_activity_at||task.updated_at):'未確認'}</p>{task.deadline_at?<p>期限：{time(task.deadline_at)}</p>:null}
