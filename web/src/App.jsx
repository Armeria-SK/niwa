...
export function App() {
  ...
  return <div className={`app-shell page-${page} ${mobileDetail ? 'mobile-detail' : ''}`} data-theme={(page === 'settings' ? previewTheme : null) || settings.theme || 'garden'}>
    ...
    {modal?.type === 'artifact' ? <ArtifactPreview onDelete={() => setModal({ type: 'delete-content', kind: 'artifact', id: modal.artifact.id, title: modal.artifact.name })} artifact={modal.artifact} members={memberMap} onClose={() => setModal(null)} onThread={openThread} /> : null}
