...
test('browser container has no host mount, network, secrets, writable OS or image-controlled entrypoint', () => {
  ...
  for (const flag of ['--network=none', '--http-proxy=false', '--read-only', '--cap-drop=ALL', '--security-opt=no-new-privileges',
    '--security-opt=seccomp=/home/niwa/niwa/config/browser-seccomp.json', '--pids-limit=256', '--memory=1g', '--memory-swap=1g', '--cpus=1', '--user=1001:1002', '--image-volume=ignore', '--pull=never', '--timeout=900']) assert.ok(args.includes(flag));
  assert.deepEqual(args.slice(-2), [environment.image, '/app/dist/entrypoints/browser-worker.js']);
  assert.ok(!args.some(arg => /--mount|--volume|--privileged|--network=host|--env-file|--no-sandbox|seccomp=unconfined|--cap-add/.test(arg)));
