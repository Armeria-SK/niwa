...
test('unsuccessful recovery is bounded across resumes and reopening, with a meaningful retry time',async t=>{
 ...
 assert.equal(calls,3);let held=r.tasks.get(admin,child.id);
 ...
 r.tasks.resume(admin,child.id);await run(r).run(r.tasks.claim(admin)!);assert.equal(calls,3);
 r.close();const reopened=new Runtime(root);t.after(()=>reopened.close());const ad=reopened.administrator();
 reopened.tasks.resume(ad,child.id);await run(reopened).run(reopened.tasks.claim(ad)!);assert.equal(calls,3);
...
test('interim reports keep the recovery budget and are not repeated while replaying saved steps',async t=>{
 ...
 for(let i=0;i<3;i++)await runner.run(r.tasks.claim(admin)!);
 assert.equal(calls,3);assert.equal(r.tasks.get(admin,child.id).state,'waiting_provider');
 assert.equal(r.messages(admin,room.id).filter(m=>m.body.startsWith('人工の途中報告')).length,3);
});
